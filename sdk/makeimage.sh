...
fdisk -u=sectors /tmp/disk.img >/dev/null << EOF
x
c
50
h
...
losetup -o $((2048*512)) /dev/loop1 /tmp/disk.img >/dev/null

# 格式化分区为 EXT3
echo -e "\033[32mFormating partition...\033[0m"
mkfs.ext3 /dev/loop1 >/dev/null

# 挂载分区
mount /dev/loop1 /mnt/img >/dev/null
mkdir -p /mnt/img/boot >/dev/null

# 复制内容
echo -e "\033[32mCopying kernel and boot configuration...\033[0m"
cp -R boot/* /mnt/img/boot/ >/dev/null
...
(hd0,1) /dev/loop1
EOF
grub-install --no-floppy --grub-mkdevicemap=/mnt/img/boot/grub/device.map --root-directory=/mnt/img /dev/loop0 >/dev/null

# clean up
echo -e "\033[32mCleaning up...\033[0m"
umount /mnt/img >/dev/null
losetup -d /dev/loop1 >/dev/null
losetup -d /dev/loop0 >/dev/null
mv /tmp/disk.img ./disk.img >/dev/null
